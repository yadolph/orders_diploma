...
`shop_list=1` - выведет список магазинов с их идентификаторами

`cat_list=1` - выведет список категорий с их идентификаторами

`page=int` - где int - положительное число - выведет страницу с соответствующим номером

`page=int`, где int - положительное число - выведет страницу с соответствующим номером

`shop_id=id`, где id - идентификатор магазина - выведет только товары данного магазина. Также можно передать название магазина в параметре `shop`, результат будет идентичен. Название магазина при этом должно точно совпадать с его названием в системе.

`cat_id=id`, где id - идентификатор категории - выведет только товары данной категории. 

Параметры `cat_id`, `shop_id` и `page` можно сочетать в одном запросе.
