...
`cat_id=int` - где int - положительное число - выведет только товары из определенной категории по id

`shop_id=int` - где int - положительное число - выведет только товары из определенного магазина по id
...
В системе предусмотрены ограничения по количеству запросов к API. Для анонимных пользователей количество запросов к API не может составлять более 15 в минуту. Для зарегистрированных пользователей - не более 60 запросов в минуту.